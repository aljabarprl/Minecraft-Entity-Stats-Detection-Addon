import { world, system, EquipmentSlot, Player } from "@minecraft/server"; // modul server
// status HUD per player
let hudEnabled = {};
let damageDealt = new Map(); // menyimpan total damage yang diterima oleh setiap entity
let lockedEntities = new Map(); // menyimpan entity yang di-lock 
// Format Entity
function formatEntityName(inputString) {
...
// Format Effects
function formatEffects(entity) {
    const effects = entity.getEffects();
    if (!effects || effects.length === 0)
        return "§7:solid_star: No Effects";
    const effectsList = effects.map((e) => `${e.typeId.replace("minecraft:", "").replace(/_/g, " ")} ${e.amplifier + 1}[${Math.floor(e.duration / 20)}s]`);
    // Maks Char
